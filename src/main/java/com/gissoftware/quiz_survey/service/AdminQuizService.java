...
public class AdminQuizService {
  ...
  public QuizInsightsDTO getQuizInsights(String quizSurveyId) {
    ...
    for (ResponseModel r : responses) {
      ...
      Integer currentScore = r.getScore();

      Integer topScore = top != null ? top.getScore() : null;
      Integer lowScore = low != null ? low.getScore() : null;

      if (top == null || (currentScore != null && (topScore == null || currentScore > topScore))) {
        top = r;
      }

      if (low == null || (currentScore != null && (lowScore == null || currentScore < lowScore))) {
        low = r;
      }

      Map<String, Object> userAnswers =
          r.getAnswers() != null ? r.getAnswers() : Collections.emptyMap();
    ...
    List<Map.Entry<String, Integer>> sortedIncorrect =
        incorrectCountPerQuestion.entrySet().stream()
            .sorted((a, b) -> b.getValue() - a.getValue())
            .toList();

    List<QuizInsightsDTO.MostIncorrectQuestionDTO> mostIncorrectQuestions =
        sortedIncorrect.stream()
            .limit(5)
            .map(
                entry -> {
                  ...
                  String questionText =
                      quiz.getDefinitionJson().getPages().stream()
                  ...
                  return new QuizInsightsDTO.MostIncorrectQuestionDTO(
    ...
    return QuizInsightsDTO.builder()
        ...
        .failRate(
            Double.parseDouble(
                String.format("%.2f", 100.0 * (totalUsers - passCount) / totalUsers)))
        .topScorer(
            top != null
                ? new QuizInsightsDTO.ScorerDTO(
                    top.getUsername(), top.getScore() != null ? top.getScore() : 0)
                : null)
        .lowestScorer(
            low != null
                ? new QuizInsightsDTO.ScorerDTO(
                    low.getUsername(), low.getScore() != null ? low.getScore() : 0)
                : null)
  ...
  public QuizCompletionStatsDTO getQuizCompletionStats(String quizId) {
    var quiz =
        quizSurveyRepo.findById(quizId).orElseThrow(() -> new RuntimeException("Quiz not found"));
    ...
    List<String> completedUserIds =
        responses.stream().map(ResponseModel::getUserId).distinct().toList();

    int totalCompleted = (int) targetedUsers.stream().filter(completedUserIds::contains).count();
    ...
    return new QuizCompletionStatsDTO(
        totalAssigned, totalCompleted, totalNotCompleted, completionRate);
  }
