...
public class ResultService {
    ...
    return responses.stream()
        ...
        .map(
            entry -> {
              ...
              List<QuizResultDTO> attemptsDTO =
  ...
  public List<SurveyResultDTO> getSurveyResultsAdmin(String quizSurveyId, String userId) {
      ...
      for (SurveyDefinition.Element el : page.getElements()) {
        ...
        switch (el.getType().toLowerCase()) {
          ...
          case "radiogroup", "checkbox", "dropdown" -> {
            ...
            List<String> choices =
                Optional.ofNullable(el.getChoices()).orElse(Collections.emptyList());
            Map<String, Integer> map = choices.stream().collect(Collectors.toMap(c -> c, c -> 0));
            counts.put(key, map);
          }
        }
      }
    ...
    for (ResponseModel resp : responses) {
      ...
      Map<String, Object> answers =
          Optional.ofNullable(resp.getAnswers()).orElse(Collections.emptyMap());
      answers.forEach(
          (key, value) -> {
  ...
  public List<SurveyResultDTO> getSurveyResultsByUserId(String quizSurveyId, String userId) {
    ...
    Map<String, Object> userAnswers =
        Optional.ofNullable(userResponse.getAnswers()).orElse(Collections.emptyMap());
    ...
    for (ResponseModel resp : responses) {
      ...
      answers.forEach(
          (key, value) -> {
            SurveyDefinition.Element el =
                definition.getPages().stream()
                    .filter(Objects::nonNull)
            ...
            if (el.getType().equalsIgnoreCase("rating")) {
              ...
              if (el.getChoices() != null) {
                counts.putIfAbsent(
                    key, el.getChoices().stream().collect(Collectors.toMap(c -> c, c -> 0)));
    ...
    return definition.getPages().stream()
        ...
        .filter(Objects::nonNull)
        .map(
            el -> buildUserSurveyResultDTO(el, counts, userAnswers, responses.size(), quizSurveyId))
        .toList();
  }

  // -------------------- HELPERS --------------------

  private SurveyResultDTO buildAdminSurveyResultDTO(
      SurveyDefinition.Element el,
    ...
    if (counts.containsKey(key)) {
      ...
      counts
          ...
          .forEach(
              (choice, count) -> {
                int percentage =
                    totalResponses > 0 ? (int) Math.round(count * 100.0 / totalResponses) : 0;
      ...
      List<ResponseModel> allResponses =
          Optional.ofNullable(responseRepo.findByQuizSurveyId(quizId))
              .orElse(Collections.emptyList());
  ...
  private SurveyResultDTO buildUserSurveyResultDTO(
      SurveyDefinition.Element el,
      Map<String, Map<String, Integer>> counts,
      Map<String, Object> userAnswers,
      int totalResponses,
    ...
    switch (type) {
      case "rating" -> {
        ...
        return new SurveyResultDTO(
            title, arabicTitle, type, Map.of("value", userValue != null ? userValue : ""));
      }
      case "radiogroup", "checkbox", "dropdown" -> {
        ...
        List<String> choices = Optional.ofNullable(el.getChoices()).orElse(Collections.emptyList());
        for (String choice : choices) {
          int count = countMap.getOrDefault(choice, 0);
          int percentage =
              totalResponses > 0 ? (int) Math.round(count * 100.0 / totalResponses) : 0;
          detailedMap.put(
              choice, Map.of("percentage", percentage, "isSelect", userSelected.contains(choice)));
        }
      ...
      case "boolean" -> {
        int yes = 0, no = 0;
        List<ResponseModel> allResponses =
            Optional.ofNullable(responseRepo.findByQuizSurveyId(surveyId))
                .orElse(Collections.emptyList());
        for (ResponseModel resp : allResponses) {
          // FIX: answers can be null
          Map<String, Object> answers =
    ...
    return new SurveyResultDTO(
        title, arabicTitle, type, Map.of("value", userValue != null ? userValue.toString() : ""));
  }
  ...
  private QuizResultDTO mapQuizResponseToDTO(
      ResponseModel resp, SurveyDefinition definition, Map<String, Object> answerKey) {
    ...
    for (SurveyDefinition.Page page : definition.getPages()) {
      ...
      for (SurveyDefinition.Element q : page.getElements()) {
        ...
        List<QuizResultDTO.OptionDTO> options =
            Optional.ofNullable(q.getChoices()).orElse(Collections.emptyList()).stream()
                .map(
                    choice -> {
