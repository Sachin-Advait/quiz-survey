...
public class FCMService {
    ...
    public void notifyOfferCreated(OfferModel offer) {
        if (offer.getTargetUsers().contains("ALL")) {
            ...
            offer.getTargetUsers().forEach(userId ->
                    userRepository.findById(userId).ifPresent(user -> sendOffer(user, offer))
            );
        }
    }
    ...
    private void sendOffer(UserModel user, OfferModel offer) {
        if (isTokenInvalid(user)) return;
        sendNotification(user.getFcmToken(), "New Offer Available 🎉", offer.getTitle(), "OFFER", offer.getId());
    }
