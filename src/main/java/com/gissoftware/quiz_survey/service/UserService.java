...
public class UserService {
  ...
  public UserModel syncUser(UserModel user) {
    ...
    Optional<UserModel> existingUser =
        userRepository.findByStaffIdAndActiveUserTrue(user.getStaffId());
  ...
  public List<UserResponseDTO> getAllUsers(
    ...
    if (page != null && size != null) {
      ...
      if (region != null && outlet != null) {
        pagedUsers =
            userRepository.findByRegionAndOutletAndActiveUserTrue(region, outlet, pageable);
      } else if (region != null) {
  ...
  public LoginResponseDTO loginByStaffId(String staffId) {

    UserModel user =
        userRepository
            .findByStaffIdAndActiveUserTrue(staffId)
            .or(() -> userRepository.findFirstByStaffIdOrderByYearDescQuarterDesc(staffId))
            .orElseThrow(() -> new RuntimeException("User not found with staffId: " + staffId));
    ...
    return LoginResponseDTO.builder()
        ...
        .staff_name(user.getUsername())
        .staff_outlet(user.getOutlet())
        .staff_region(user.getRegion())
        .staff_position(user.getPosition())
        .build();
  }
}
