...
import com.gissoftware.quiz_survey.dto.QuizDataDTO;
...
public class UserScoreService {
  ...
  public UserSingleScoreDTO calculateUserScore(String userId) {
    ...
    List<QuizDataDTO> quizData =
        bestAttempts.values().stream()
            .map(
                r -> {
                  QuizSurveyModel quiz = quizSurveyRepo.findById(r.getQuizSurveyId()).orElse(null);

                  double percentage = 0;

                  if (r.getMaxScore() != null && r.getMaxScore() > 0) {
                    percentage = (r.getScore() * 100.0) / r.getMaxScore();
                  }

                  return new QuizDataDTO(
                      r.getQuizSurveyId(),
                      quiz != null ? quiz.getTitle() : null,
                      quiz != null ? quiz.getCreatedAt() : null,
                      round(percentage));
                })
            .toList();
  ...
  private double calculateSurveyParticipation(String userId) {
    ...
    List<QuizSurveyModel> assignedSurveys = quizSurveyRepo.findByTypeIgnoreCase("survey");

    if (assignedSurveys.isEmpty()) return 0.0;

    long assignedCount =
        assignedSurveys.stream().filter(s -> s.getTargetedUsers().contains(userId)).count();
    ...
    long participatedCount =
        assignedSurveys.stream().filter(s -> respondedSurveyIds.contains(s.getId())).count();
  ...
  private double calculateSatisfactionScore(List<ResponseModel> responses) {
    ...
    for (ResponseModel response : responses) {
      QuizSurveyModel survey = quizSurveyRepo.findById(response.getQuizSurveyId()).orElse(null);
