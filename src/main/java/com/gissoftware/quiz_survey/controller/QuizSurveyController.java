...
public class QuizSurveyController {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<ApiResponseDTO<QuizSurveyDTO>> getQuizSurvey(
      @PathVariable String id, @RequestParam String userId) {
    QuizSurveyDTO quizSurvey = quizSurveyService.getQuizSurvey(id, userId);
    if (quizSurvey == null) {
      return ResponseEntity.status(404)
          .body(new ApiResponseDTO<>(false, "Quiz & survey not found", null));
    }
    return ResponseEntity.ok(
        new ApiResponseDTO<>(true, "Quiz & survey retrieved successfully", quizSurvey));
  }
  ...
  public ResponseEntity<ApiResponseDTO<PageResponseDTO<QuizzesSurveysDTO>>> getQuizzesSurveys(
    ...
    PageResponseDTO<QuizzesSurveysDTO> surveys =
        quizSurveyService.getQuizzesSurveys(
            userId, status, type, sort, participation, startDate, page, size);
    return ResponseEntity.ok(
        new ApiResponseDTO<>(true, "All quiz & surveys retrieved successfully", surveys));
  }

  @GetMapping("/summary/{quizSurveyId}")
  public ResponseEntity<ApiResponseDTO<QuizScoreSummaryDTO>> getSummary(
